...
<%  Option Explicit
	...
	' RespType está preparado para administrarse, pero no se usa en el formulario, así que está desactivado
	dim mostrarRespType: mostrarRespType = FALSE
...
Sub saveNewQuestion_click()
    ...
    if Request.Form("newQuestText")<>"" AND Request.Form("newQuestWeight")<>"" AND isNumeric(Request.Form("newQuestWeight")) then
        ...
        if questionModified then
            recalculateFormKPIQuality idForm
        end if
...
Sub saveNewResponse_click(idQuest, idResp)
    dim sResponseText, sResponseValue
    dim responseModified
...
select case EventObject
	...
	case "saveNewResponse" saveNewResponse_click EventParam1, EventParam2
...
<title><%=IDM_AdminFormTitle %></title>
	...
	<script language="JavaScript">
		...
		function addResponse(tr, oIdResp, oValue, oText){
		    oIdResp.value = '';
		    oValue.value = '';
		    oText.value = '';
		    tr.style.display = '';
		...
		function editResponse(oIdResp, idResp, tr, oRespValue, oRespText, value, text){
		    
		    oIdResp.value = idResp;
		    oRespValue.value = value;
		    oRespText.value = text;

		    tr.style.display = '';
		    oRespValue.focus();
		    
		}
		function cancelAddResponse(tr, oIDResp, oValue, oText){
		    tr.style.display = 'none';
		    oIDResp.value = '';
		    oValue.value = '';
		    oText.value = '';
		}
		function saveNewResponse(idQuest, idResp, oValue, oText){
		    if (oValue.value == '' || oText.value == '') {alert('<%=IDM_JS_RespTextValue %>'); return false;}
		    
		    _fireEvent('saveNewResponse', idQuest, idResp);
		}
...
<FORM method=post name="thisForm">
	...
	<table border=0 cellpadding=0 cellspacing=0 width="600px" style="border:1px solid gray;" align=center><tr height="400px"><td valign="top" style="border-right:2px solid black;border-bottom:2px solid black;">
	    ...
	    <tr>
		    ...
		    <td align=right>
		        <img onclick="Save();" title="<%=IDM_MenuSaveForm %>" style="cursor:pointer;border:2px solid white;" onmouseover="this.style.border = '2px solid gray';" onmouseout="this.style.border = '2px solid white';" src="images/save.png" />
		    </td>
		</tr>
		</table>
                ...
                <div id="DIV_NewQuestion" style="display:none;position:absolute;left:300px;top:150px;border:1px solid gray;width:400px;height:100px;background-color:White;">
                    ...
                    <table width="100%" cellpadding="3" cellspacing="0">
                        <tr>
                            ...
                            <%if mostrarRespType then %>
                                <td><font class="font10"><%=IDM_RespType %></font></td>
                            <%end if %>
                        </tr>
                        <tr>
                            <td><input type="text" name="newQuestText" value="" style="width:280px;"  /> </td>
                        ...
                        <tr>
                            ...
                            <tr>
                                ...
                                <td style="padding-left:20px;">
                                    <table width="100%" cellpadding="0" cellspacing="0">
                                        <tr height="25">
                                            <td width="35"><%if canModify then %><a title="<%=IDM_CreateResponse %>" href="" onclick="addResponse(TR_NewResp_<%=qst.ID %>, editIdResp_<%=qst.ID %>, newRespValue_<%=qst.ID %>, newRespText_<%=qst.ID %>);return false;"><img src="images/nuevo.png" style="border:0px;" /></a><%end if %></td>
                                        ...
                                        <%for each rsp in qst.Responses %>
                                            <tr>
                                                <td><%if canModify then %>
                                                    ...
                                                    <%end if %>
                                                    <a title="" href="" onclick="editResponse(editIdResp_<%=qst.ID %>, '<%=rsp.ID %>', TR_NewResp_<%=qst.ID %>, newRespValue_<%=qst.ID %>, newRespText_<%=qst.ID %>, '<%=rsp.RespValue %>', '<%=server.htmlencode(replace(replace(rsp.Text,"\","\\"),"'","\'")) %>');return false;"><img src="images/edit.png" style="border:0px;" /></a>
                                                </td>
                                                <td width="50" align="center"><font class="font12"><%=rsp.RespValue %></td>
                                                <td><font class="font12"><%=rsp.Text %></td>
                                            </tr>
                                        <%next %>
                                        <%if TRUE OR canModify then %>
                                            <input type="hidden" name="editIdResp_<%=qst.ID %>" value="" />
                                            <tr id="TR_NewResp_<%=qst.ID %>" style="display:none;">
                                                <td>
                                                    <a title="<%=IDM_Cancel %>" href="" onclick="cancelAddResponse(TR_NewResp_<%=qst.ID %>, editIdResp_<%=qst.ID %>, newRespValue_<%=qst.ID %>, newRespText_<%=qst.ID %>);return false;"><img src="images/cancel.png" style="border:0px;" /></a>
                                                    <a title="<%=IDM_SaveResponse %>" href="" onclick="saveNewResponse('<%=qst.ID %>', editIdResp_<%=qst.ID %>.value, newRespValue_<%=qst.ID %>, newRespText_<%=qst.ID %>);return false;"><img src="images/ok.png" style="border:0px;" /></a>
                                                </td>
