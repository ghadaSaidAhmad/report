...
Dim Formulario: Set Formulario = New ASPForm %><!--#INCLUDE FILE="include/UploadClass.asp"--><% 
Server.ScriptTimeout = 2000
Formulario.SizeLimit = &HA00000
Dim MaxFileSize: MaxFileSize = 1024*1024*3

If Formulario.State = 0 Then 'Completed
	Dim File
	
    EventObject = Formulario.Item("EventObject")
    EventParam1 = Formulario.Item("EventParam1")
    EventParam2 = Formulario.Item("EventParam2")
    CloseWindow = Formulario.Item("CloseWindow")

    Name = Formulario.Item("Name")
    indBaja = Formulario.Item("indBaja")

	'For Each File In Formulario.Files
	dim iItem: iItem = 0
	dim fso: set fso = Server.CreateObject("Scripting.FileSystemObject")
	
	For each File in Formulario.Files.Items
        ...
        iItem = iItem + 1
    Next
ElseIf Formulario.State > 10 then
	Const fsSizeLimit = &HD
	Select case Formulario.State
		case fsSizeLimit: 
			Response.Write "<script language=""JavaScript"">alert('Tamaño máximo del fichero sobrepasado " & Formulario.TotalBytes & "B (Máximo " & Formulario.SizeLimit & "B)');history.back(-1);</script>"
			Response.End
		case else 
