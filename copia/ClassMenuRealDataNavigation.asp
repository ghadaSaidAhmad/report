    ...
    <table width="100%" bgcolor="<%=Application("ColorSelectorDark")%>" cellpadding="1" cellspacing="0">
        <tr>
            ...
            <td width=250 align="right" style="padding-right:10px;">
                <input type="checkbox" name="ViewNShops" <%if (Request.Form("ViewNShops")<>"" AND Request.Form("PageReloaded")<>"") OR Request.Form("PageReloaded")="" then %>checked<%end if %> onclick="if (checkChanges()){ _fireEvent('ChSel','',''); }else{this.checked = !this.checked}" /><font class="font12" color="white"><%=Left(IDM_NTiendas, 7) %></font>
                <input type="checkbox" name="ViewPercentComplaint" <%if (Request.Form("ViewPercentComplaint")<>"" AND Request.Form("PageReloaded")<>"") OR Request.Form("PageReloaded")="" then %>checked<%end if %> onclick="if (checkChanges()){ _fireEvent('ChSel','',''); }else{this.checked = !this.checked}" /><font class="font12" color="white"><%=Left(IDM_PercentComplaint, 7) %></font>
            </td>
            <td>
